...
function buildY4M() {
  ...
  console.log(
    `[p6] wrote ${W}x${H}, ${count} frames (${mb} MB): ` +
...
try {
  ...
  // Take the device's path, all the way. On a phone Phase 6 is reached from a TRACKING screen
  ...
  // Phase 5's own screen, entered and started, because Phase 6 inherits a *running* verifier
  // and the anchor it holds. Entering Phase 6 over a cold Phase 5 would exercise a sequence no
  // device ever takes — the same §H.5 lesson, one screen further along.
  ...
  const gate = await page.evaluate(() => {
    const b = document.getElementById('go-to-phase6');
    return { text: b?.textContent ?? null, disabled: b?.disabled ?? null };
  });
  if (gate.disabled !== true || !String(gate.text).includes('LOCKED')) {
    throw new Error(
      'GO TO RELATIVE POSE should be locked on this leg — Phase 5 is TESTING, not PASSED — ' +
        `but it reads ${JSON.stringify(gate.text)} (disabled ${gate.disabled}). Rule 005.`,
    );
  }
  ...
  const before = await page.evaluate(() => {
    const b = document.getElementById('start-pose');
    return { text: b?.textContent ?? null, disabled: b?.disabled ?? null };
  });
  if (before.disabled !== false || before.text !== 'START POSE RECOVERY') {
    throw new Error(
      `START POSE RECOVERY is not pressable on arrival: label ${JSON.stringify(before.text)}, ` +
        `disabled ${before.disabled}. Phase 6 arrives over five live stages — camera, pipeline, ` +
        'detector, tracker and verifier — so a control derived from any of them is already ' +
  ...
  if (after.text !== 'RECOVERING' || after.disabled !== true) {
    throw new Error(
      `pose recovery is running but the control says ${JSON.stringify(after.text)} ` +
