...
on this device on this build, or the スキャン開始 control stays locked — a pass from an earlier page
...
1. On the 「環境チェック」画面, tap **スキャン開始**. It is enabled only because Phase 0 passed.
2. Tap **カメラ開始** and allow camera access.
3. **Hold the camera open for at least 35 seconds without leaving Safari.**
   - The *連続* field counts up to 30.0 s. CAM-003 stays `PENDING` until it fills.
...
4. **Move the phone around** while it counts. Pan across the room slowly.
   - Watch *画像変化 Δ 最大* rise. CAM-004 needs it above **8.0**; pointing at a motionless
     wall only reaches 1–3, which is why the test asks you to move.
   - *ノイズ下限* shows the median for comparison.
...
6. When the verdict panel shows what you want, tap
   **エビデンス JSON をダウンロード**. The verdict is in the filename.
...
1. In Safari, tap the **ăA** menu in the address bar → **Website Settings** → **Camera** →
   **Deny**.
2. Reload the page, go to the 「環境チェック」画面, and tap **カメラ開始**.
3. Expect: `カメラの許可が拒否されました`, no image at all, and the error with its recovery
   action. CAM-002 turns `PASS`.
4. Export the evidence and screenshot again.
5. Set the camera permission back to **Ask** afterwards.

The screen will show CAM-001 as `前回から引き継ぎ` in run 2 (and CAM-002 as `前回から引き継ぎ` if you go back
...
| CAM-002 shows `前回から引き継ぎ` in the granted run | The denial was observed in the other run. Both bundles still have to be committed. |

## What Phase 1 does not do

No frame pipeline, no feature detection, no pose, no spatial data. The 「環境チェック」画面 shows a
